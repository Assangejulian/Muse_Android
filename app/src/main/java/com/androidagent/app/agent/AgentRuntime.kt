...
class AgentRuntime(
    ...
    suspend fun run(): RuntimeResult {
        ...
        val visionKey = settings.visionApiKey.ifBlank {
            if (settings.currentProvider == "qwen") apiKey else ""
        }
        ...
        for (step in 1..MAX_TOOL_CALLS) {
            ...
            if (milestone == null) {
                ...
                val revised = replan(plan, ledger, immutableGoal, appCatalog, targetHint, canonicalQuery, actorKey, actorBaseUrl, actorModel, verification.reason)
                plan = revised.preserveCompletedPrefix(plan, completedCount)
                ledger.replacePlan(plan, completedCount)
                guard = ToolGuard(plan)
                replans += 1
            ...
            if (cycle != null || ledger.noProgressCount >= MAX_NO_PROGRESS) {
                ...
                onPhase(step, "Replanning")
                val completedCount = ledger.currentMilestoneIndex
                val revised = replan(plan, ledger, immutableGoal, appCatalog, targetHint, canonicalQuery, actorKey, actorBaseUrl, actorModel)
                plan = revised.preserveCompletedPrefix(plan, completedCount)
                ledger.replacePlan(plan, completedCount)
                guard = ToolGuard(plan)
                replans += 1
            ...
            traceStore.event(runId, "TOOL_PROPOSED", mapOf("step" to step, "milestone" to milestone.id, "action" to proposed.toString(), "basedOn" to before.observationId))
            ...
            if (proposed is AgentAction.Finish) {
                ...
                ledger.record(StepTrace(milestone.id, before.observationId, proposed.toString(), before.observationId, TransitionJudgement.NO_PROGRESS, "Actor cannot satisfy the Stop Gate"))
            ...
            if (proposed is AgentAction.Fail) {
                ...
                ledger.record(StepTrace(milestone.id, before.observationId, proposed.toString(), before.observationId, TransitionJudgement.NO_PROGRESS, proposed.reason))
                continue
            }
            val guarded = guard.normalizeAndValidate(proposed, before)
            ...
            val criticCompletionAllowed = rawCritic.judgement == TransitionJudgement.MILESTONE_COMPLETE &&
                hardAfter.proven &&
                (togglePredicates.isEmpty() || toggleAfter.proven || afterScreenshot != null)
            val critic = when {

...
object TraceSanitizer {
    ...
    fun action(action: AgentAction): String = when (action) {
        ...
        is AgentAction.FindNodes -> "find_nodes(constraint=${action.query.hasConstraint()}, limit=${action.query.limit})"
        is AgentAction.ReadNode -> "read_node(nodeId_present=${action.nodeId != null}, ${selectorMetadata(action.selector)})"
        is AgentAction.ScrollUntil -> "scroll_until(direction=${action.direction}, maxSwipes=${action.maxSwipes})"
        is AgentAction.WaitUntil -> "wait_until(milliseconds=${action.milliseconds})"
        is AgentAction.Terminal -> "terminal(command_chars=${action.command.length}, command_sha256=${digest(action.command)}, timeout=${action.timeoutMillis})"
    ...
    fun actionTarget(action: AgentAction, @Suppress("UNUSED_PARAMETER") observation: Observation? = null): String = when (action) {
        ...
        is AgentAction.ReadNode -> "read_target(nodeId_present=${action.nodeId != null}, ${selectorMetadata(action.selector)}, type=node)"
        is AgentAction.FindNodes, is AgentAction.ScrollUntil, is AgentAction.WaitUntil -> "query_target(constraint=true, type=query)"

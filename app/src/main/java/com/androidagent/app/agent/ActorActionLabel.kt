...
internal object ActorActionLabel {
    fun describe(action: AgentAction, target: UiNodeSnapshot? = null): String = when (action) {
        ...
        is AgentAction.FindNodes -> "find_nodes(${action.query.signature().take(40)})"
        is AgentAction.ReadNode -> "read_node(#${action.nodeId ?: 0})"
        is AgentAction.ScrollUntil -> "scroll_until(${action.direction})"
        is AgentAction.WaitUntil -> "wait_until(${action.milliseconds}ms)"

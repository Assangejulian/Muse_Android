...
The application ID remains `com.androidagent.app`; version `0.17.6` uses versionCode `59` for in-place updates. Environment setup bootstraps `ca-certificates` over an APT-signed HTTP index before switching the selected mirror back to HTTPS for all remaining packages.

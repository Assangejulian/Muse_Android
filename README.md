...
The application ID remains `com.androidagent.app`; version `0.17.4` uses versionCode `57` for in-place updates. Environment setup bootstraps `ca-certificates` over an APT-signed HTTP index before switching the selected mirror back to HTTPS for all remaining packages.

...
A frosted-glass progress overlay sits at the top of the screen (`TYPE_ACCESSIBILITY_OVERLAY`, blur-behind on Android 12+) with the model's raw Chinese thought, action budget, and Pause / Skip / Abort chips. Volume down pauses or resumes; volume up skips the current approach. Changing accessibility flags may require toggling Muse in system accessibility settings once after install.
...
The application ID remains `com.androidagent.app`; version `0.18.2` uses versionCode `63` for in-place updates. Environment setup bootstraps `ca-certificates` over an APT-signed HTTP index before switching the selected mirror back to HTTPS for all remaining packages.

...
The application ID remains `com.androidagent.app`; version `0.17.5` uses versionCode `58` for in-place updates. Environment setup bootstraps `ca-certificates` over an APT-signed HTTP index before switching the selected mirror back to HTTPS for all remaining packages.
